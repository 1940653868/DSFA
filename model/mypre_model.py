...
class ChanSeqPosiionEmbedding(nn.Module):
    def __init__(self, ch_num, d_model):
        ...
        # self.channel_pos_encoding = nn.Embedding(ch_num, d_model)
...
# class EmbedingAndMask(nn.Module):
...
class TimeEncoder(nn.Module):
    def __init__(self, in_dim, d_model, dim_feedforward, patch_num, n_layer, nhead, band_num, project_mode, learnable_mask, args=None, postion=None):
        ...
        # N层transformer
        enc_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward, batch_first=True, dropout=args.atten_dropout)
...
class ChannelEncoder(nn.Module):
    def __init__(self, out_dim, d_model, dim_feedforward, n_layer, nhead, atten_dropout = 0.2):
        super(ChannelEncoder, self).__init__()

        enc_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward, batch_first=True, dropout=atten_dropout)
        self.trans_enc = nn.TransformerEncoder(enc_layer, num_layers=n_layer)
        self.apply(_weights_init)
...
class TimeseqUnfoldProject(nn.Module):
    ...
    def forward(self, x):
        x_unfold = reshape_tensor_with_overlap(input_tensor=x, seg_len=self.seg_len,
                    stride=self.stride)
        output = self.ProjectLayer(x_unfold) #batch,channel, block,seg_len->emb_size
...
class Brant_encoder(nn.Module):
    def __init__(self, args) -> None:
        ...
        # sft-project + time-project --> Time_encod(project-positionembeddingt-transformer)-->
        ...
        hz_size = 32
        self.cwt_projection = CwtPatchEmbed(img_size=(hz_size, args.seq_len), patch_size=(hz_size, 50), stride=(hz_size, 50), 
                                        in_chans=args.ch_num, embed_dim_ratio=16, emb_size=emb_size)
        #-----------------------------------------------------
        ...
        self.decoder_pos_embed = SeqPositionalEncoding(self.d_model) #略好于上一个方案，收敛域0.999?
        self.decoder_pos_embed = position
        self.decoder = TransformerLayers(hidden_dim= args.d_model, nlayers=args.decoder_layer, mlp_ratio=1, num_heads=8, dropout=0.1) #dropout 1 what the fuck
    ...
    def stft(self, sample, device = 'cuda'):
        spectral = torch.stft( 
            ...
            window = torch.hamming_window(window_length=self.hop_length).to(device), # 新添加的
            center = False,
            onesided = True,
            return_complex = True,
            normalized=False
    ...
    def forward(self, batch, use_power,
        ...
        time_seq = batch[0]
        power = batch[1]
        if self.encdata_mode in ['freq_only','both']:
            use_sft = True
        elif self.encdata_mode in ['time_only' ,'cwt']:
            use_sft = False
        else:
            use_sft = True
        ...
        if use_sft: #计算power用于下游
            ...
            for i in range(x.shape[1]): # x.shape ([batch, channel, time_seq])
                channel_spec_emb, channel_angle_emb = self.stft(x[:, i : i + 1, :], device=self.args.device)            # [batch, fre_num, time_blcok_num]
            ...
            emb  = emb.view(batch_size, channel_num, -1, self.emb_size)
            init_fre_emb = torch.cat(init_power, dim=1)
            init_fre_emb = init_fre_emb.view(batch_size, channel_num, -1, self.emb_size)
        ...
        elif self.encdata_mode == 'cwt':   
            data = power #data shape[batch, channel, sample_step(2000), hz_rate(32,100)]
            data = self.cwt_projection(data)##data shape[batch, channel, sample_step(2000), feature]
        else:
            raise f'noy implement encode mode{self.encdata_mode}, please check encdata_mode'
        ...
        batch_size, ch_num, patch_num, d_model = time_z.shape    
        # channel encoder
        ch_out = self.ch_enc(time_z)          # batch_size, ch_num, patch_num, -1   
        ...
        if self.train_mode == 'pretrain':
            ...
            ch_out = torch.mean(ch_out, dim=-2) # batch_size, patch_num, ch_num, -1
            return ch_out
...
def set_args_func(return_parser = False):
    import argparse   
    ...
    parser.add_argument("--d_model", type=int, default=256)#320 #1280
    parser.add_argument('--train_mode', type=str, default = 'finetune') #finetune pretrain
    parser.add_argument("--encdata_mode", type=str, default='freq_only', choices=['time_only','freq_only','both','cwt'])
    seq_len = 1125
    seg_len = 45
    seq_len = 2000
    seg_len = 200 #50
    stride = 100  #$seg_len
    parser.add_argument("--dim_feedforward", type=int, default=256)#320#3072
    parser.add_argument("--seq_len", type=int, default = seq_len)
    parser.add_argument("--seg_len", type=int, default = seg_len)
    parser.add_argument("--stride", type=int, default = stride)
    parser.add_argument("--ch_num", type=int, default=16)#22
    patch_num = int((seq_len-seg_len)/stride+1)
    parser.add_argument("--patch_num", type=int, default=patch_num)
    parser.add_argument("--time_ar_layer", type=int, default=3)#4
    parser.add_argument("--time_ar_head", type=int, default=8)#16
    parser.add_argument("--input_emb_mode", type=str, default='linear')
    parser.add_argument("--ch_ar_layer", type=int, default=3)#4
    parser.add_argument("--ch_ar_head", type=int, default=8)#16
    parser.add_argument("--decoder_layer", type=int, default=4)#4
    parser.add_argument("--atten_dropout", type=float, default=0.2)#4

    parser.add_argument("--learnable_mask", type=str2bool, default=False)
    
    parser.add_argument('--enable_down_stram', type=str2bool, default = True) #False
    ...
    parser.add_argument("--time_lr", type=float, default=1e-6)
    ...
    # 超参数部分
    parser.add_argument("--batch_size", type=int, default=64)#64 batch_size=64收敛速度快， 128稍微慢,但是目前进度76.15
    if return_parser:
        return parser
    else:
...
if __name__ =="__main__":
    ...
    if args.encdata_mode == 'cwt':
        hz_samplerate = 32
        input_size = (2, batch, ch_num,args.seq_len,hz_samplerate)
        batch_data = torch.ones(2, batch, ch_num,args.seq_len,hz_samplerate)
    else:
    ...
    # python model/mypre_model.py --encdata_mode cwt